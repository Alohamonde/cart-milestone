const FUNCTION_HANDLE = "milestone-discount";
const DISCOUNT_TITLE = "Cart Milestone Reward";

const LIST_DISCOUNTS = `#graphql
  query MilestoneDiscountList {
...
const CREATE_DISCOUNT = `#graphql
  mutation MilestoneDiscountCreate($automaticAppDiscount: DiscountAutomaticAppInput!) {
...
const UPDATE_DISCOUNT = `#graphql
  mutation MilestoneDiscountUpdate($id: ID!, $automaticAppDiscount: DiscountAutomaticAppInput!) {
...
function isOurDiscount(discount: {
  ...
  return (discount.title || "").includes("Cart Milestone");
}

export async function getMilestoneDiscountStatus(
...
export async function ensureMilestoneDiscount(
  admin: Parameters<typeof safeGraphql>[0],
) {
  const current = await getMilestoneDiscountStatus(admin);
  ...
  if (current.exists) {
    ...
    const after = await getMilestoneDiscountStatus(admin);
